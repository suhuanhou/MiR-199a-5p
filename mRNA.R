...
if(T){
  ...
  group_List = meta[which((meta[,2]) %in% colnames(exprSet)),8]
}
...
ids = ids[match(rownames(exprSet),ids$probe_id),]  # filter
...
jimmy <- function(exprSet,ids){
  ...
  print(dim(exprSet))  # filter before
  exprSet = exprSet[rownames(exprSet) %in% probes,]
  
  print(dim(exprSet))  # filter after
...
exprSet = new_exprSet
# write.csv(exprSet, "mRNA_expression.csv")


if(T){  
  group_list = group_List
  library(reshape2)
  a = exprSet  # transformation
  a$"gene"=rownames(a)
...
write.table(re, file = paste0(path,'/DEmRNA.csv'),row.names=TRUE,col.names=TRUE,sep=",")
